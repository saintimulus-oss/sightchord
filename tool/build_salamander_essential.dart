...
Future<void> main() async {
  ...
  final selectedRegions = noteRegions
      .where(
        (region) =>
      ...
      .toList(growable: false)
    ..sort((left, right) {
      final keyCompare = left.pitchKeycenter.compareTo(right.pitchKeycenter);
      if (keyCompare != 0) {
        return keyCompare;
      }
  ...
  for (final sampleRelativePath in selectedSampleNames) {
    final sourceFile = _fileFromLibraryPath(_sourceLibraryUri, sampleRelativePath);
    final outputFile = _fileFromLibraryPath(_outputLibraryUri, sampleRelativePath);
  ...
  await File.fromUri(_outputLibraryUri.resolve('manifest.json')).writeAsString(
    const JsonEncoder.withIndent('  ').convert(manifest),
  );
  await File.fromUri(_outputLibraryUri.resolve('stats.json')).writeAsString(
...
List<_SfzRegion> _parseSfz(List<String> lines) {
  ...
  for (final line in lines) {
    ...
    for (final token in trimmed.substring('<region>'.length).trim().split(' ')) {
...
String _buildSimplifiedSfz(List<Map<String, Object?>> regions) {
  final buffer = StringBuffer()
    ...
    ..writeln('// Learning-focused Salamander subset for SightChord')
...
class _SfzRegion {
  ...
  bool get isNoteSample => sampleRelativePath.startsWith('samples/') && layer > 0;
